...
def calc_params(layer_sizes, base_num):
    total_params = 0

    for id in range(len(layer_sizes)-1):
        total_params += (layer_sizes[id] * layer_sizes[id+1])* base_num + layer_sizes[id+1]  # weights + biases
    return total_params

# class HyperNetwork(nn.Module):
#     def __init__(self, input_dim, hidden_dim, output_dim, dropout_rate=0.1, spect_norm=True):
#         """
#         Parameters:
#             input_dim (int): Dimension of the input layer.
#             hidden_dim (list of int): List of dimensions for each hidden layer.
#             output_dim (int): Dimension of the output layer.
#             dropout_rate (float): Dropout rate for regularization.
#             spect_norm (bool): Whether to apply spectral normalization to layers.
#         """
...
#             if spect_norm:
#                 self.layers.append(spectral_norm(nn.Linear(prev_dim, hidden)))
#             else:
...
#         # Ensure that the last hidden layer size is compatible
#         assert prev_dim % output_dim == 0, "Last hidden layer size must be divisible by output_dim"
#         self.chunk_size = prev_dim // output_dim  # Should be 10 if prev_dim=90 and output_dim=9
        
#         # Define the weight matrix of size (10, 1) for the last layer
#         self.W_last = nn.Parameter(torch.randn(self.chunk_size, 1))
...
#         # Initialize W_last
#         nn.init.xavier_normal_(self.W_last)

#     def forward(self, x):
#         for layer in self.layers:
#             x = layer(x)
#         # x shape: (batch_size, prev_dim)
#         batch_size = x.size(0)
        
#         # Reshape x to (batch_size, output_dim, chunk_size)
#         x = x.view(batch_size, -1, self.chunk_size)
        
#         # Multiply each 10-dimensional vector with the weight matrix
#         # x: (batch_size, output_dim, chunk_size)
#         # W_last: (chunk_size, 1)
#         # Resulting x: (batch_size, output_dim, 1)
#         x = torch.matmul(x, self.W_last)
        
#         # Squeeze the last dimension to get (batch_size, output_dim)
#         x = x.squeeze(-1)
#         return x
...
class TaskHyperNet(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim):
        ...
        self.projected_task_embedding_dim = self.task_hidden_dim = hidden_dim
...
# class HyperNetwork(nn.Module):
#     def __init__(self, input_dim, hidden_dim, output_dim, dropout_rate=0, spect_norm=1):
#         super(HyperNetwork, self).__init__()
#         self.layers = nn.ModuleList()
#         if spect_norm:
#             self.layers.append(spectral_norm(nn.Linear(input_dim, hidden_dim)))
#         else:
#             self.layers.append(nn.Linear(input_dim, hidden_dim))
#         self.layers.append(nn.ReLU())
#         self.layers.append(nn.Dropout(dropout_rate))
#         if spect_norm:
#             self.layers.append(spectral_norm(nn.Linear(hidden_dim, output_dim)))
#         else:
...
class MultKAN(nn.Module):
    ...
    -----------
        ...
        device : str
    '''
    def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun='silu', symbolic_enabled=True, affine_trainable=False, grid_eps=0.02, grid_range=[-1, 1], sp_trainable=False, sb_trainable=False, seed=1, save_act=True, sparse_init=False, auto_save=True, first_init=True, ckpt_path='./model', state_id=0, round=0, device='cpu', embedding_dim=3, hidden_dim=32, dropout_rate = 0, spect_norm = False):
        '''
        ...
        self.width = width
        # self.act_hypernet = TaskHyperNet(input_dim, 256).to(device)
        # self.hypernet = linear_layer(256,output_dim).to(device)
        self.hypernet = HyperNetwork(input_dim, hidden_dim, output_dim).to(device)
         # 定义两个超网络
        # self.hypernet1 = HyperNetwork(input_dim, hidden_dim, output_dim, dropout_rate=dropout_rate, spect_norm=spect_norm).to(device)
        # self.hypernet2 = HyperNetwork(input_dim, hidden_dim, output_dim, dropout_rate=dropout_rate, spect_norm=spect_norm).to(device)
        ...
        for l in range(self.depth):
            ...
            # layer_embedding = torch.randn(1, width_in[l], width_out[l+1], embedding_dim, device=device)
            # nn.init.xavier_normal_(layer_embedding[0, :, :, :])
            # embedding = layer_embedding.expand(width_in[l] * width_out[l+1], embedding_dim).clone()
            # nn.init.xavier_normal_(embedding)
            # # Convert to nn.Parameter and add to the embeddings list
            # embedding_param = nn.Parameter(embedding).to(device)
            # self.embeddings.append(embedding_param)
            # splines
            if embedding_dim <4:
                # embedding parameters
                embedding = nn.Parameter(torch.randn(width_in[l], width_out[l+1], embedding_dim)).to(device)
                nn.init.xavier_normal_(embedding)
            else:
                # Initialize the shared first dimension for the entire layer
                e0 = torch.randn(2, device=device)
                # Initialize the second dimension shared among the same input nodes
                e1 = torch.randn(width_in[l], device=device)
                # Initialize the third dimension shared among the same output nodes
                e2 = torch.randn(width_out[l+1], device=device)
                
                # Create the embedding tensor with the specified structure
                embedding = torch.zeros(width_in[l], width_out[l+1], 4, device=device)
                embedding[:, :, 0] = e0[0]  # First dimension is the same for the entire layer
                embedding[:, :, 1] = e0[1]
                embedding[:, :, 2] = e1.unsqueeze(1)  # Second dimension is the same for the same input nodes
                embedding[:, :, 3] = e2.unsqueeze(0)  # Third dimension is the same for the same output nodes
                nn.init.xavier_normal_(embedding)
    ...
    def forward(self, x, singularity_avoiding=False, y_th=10.):
        ...
        for l in range(self.depth):
            ...
            # weights = self.hypernet(self.act_hypernet(self.embeddings[l].view(-1,self.embedding_dim))).view(self.width_in[l], self.width_out[l+1], self.base_num)
            # if l < self.depth // 2:
            #     weights = self.hypernet1(self.embeddings[l].view(-1, self.embedding_dim)).view(self.width_in[l], self.width_out[l+1], self.base_num)
            # else:
            #     weights = self.hypernet2(self.embeddings[l].view(-1, self.embedding_dim)).view(self.width_in[l], self.width_out[l+1], self.base_num)
    ...
    def get_active_neurons(self, threshold=1e-2, mode="auto", active_neurons_id=None):
        ...
        -----
            threshold : float
                如果神经元的归因分数低于该阈值，则认为其不活跃。
            mode : str
                'auto' 或 'manual'。'auto' 模式使用阈值自动筛选活跃节点，'manual' 模式则要求传入 active_neurons_id。

        Returns:
        --------
            active_neurons_up : list
                上层活跃神经元的索引列表。
            active_neurons_down : list
                下层活跃神经元的索引列表。
        '''
        ...
        return active_neurons_up, active_neurons_down
