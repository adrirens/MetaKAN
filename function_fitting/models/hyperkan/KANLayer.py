...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.5, scale_base_mu=0.0, scale_base_sigma=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, 1], sp_trainable=False, sb_trainable=False, save_plot_data = True, device='cpu', sparse_init=False):
        ''''
        ...
        self.coef = curve2coef(self.grid[:,k:-k].permute(1,0), noises, self.grid, k)
        ...
        self.scale_base = scale_base_mu * 1 / np.sqrt(in_dim) + \
                         scale_base_sigma * (torch.rand(in_dim, out_dim)*2-1) * 1/np.sqrt(in_dim)

        self.scale_sp = torch.ones(in_dim, out_dim) * scale_sp * self.mask # make scale trainable
    ...
    def forward(self, x, weights):
        ...
        self.coef = weights[:,:,:self.num + self.k]   
        base = self.base_fun(x) # (batch, in_dim)
        y = coef2curve(x_eval=x, grid=self.grid, coef=self.coef, k=self.k)
        
        postspline = y.clone().permute(0,2,1)
        self.scale_base = weights[:,:,-1]
    ...
    def get_subset(self, in_id, out_id):
        ...
        spb.coef.data = self.coef.to(self.device)[in_id][:,out_id]
        spb.scale_base.data = self.scale_base.to(self.device)[in_id][:,out_id]
        spb.scale_sp.data = self.scale_sp.to(self.device)[in_id][:,out_id]
