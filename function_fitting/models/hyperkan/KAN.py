...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun=torch.nn.SiLU(), symbolic_enabled=True, bias_trainable=False, grid_eps=1.0, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True,
                 device='cpu', seed=0, embedding_dim=1, hidden_dim=32, dropout_rate = 0, spect_norm = False):
        '''
        ...
        for l in range(self.depth):
            # embedding
            embedding = nn.Parameter(torch.randn(width[l], width[l+1], embedding_dim)).to(device)
    ...
    def forward(self, x):
        ...
        for l in range(self.depth):
            weights = self.hypernet(self.embeddings[l].view(-1,self.embedding_dim)).view(self.width[l], self.width[l+1], self.base_num)
    ...
    def plot(self, folder="./figures", beta=3, mask=False, mode="supervised", scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None):
        ...
        plt.savefig(f'{folder}/skeleton.png', bbox_inches="tight", dpi=400)

    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=0., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
