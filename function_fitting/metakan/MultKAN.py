...
class MultKAN(nn.Module):
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1.,start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
        ...
        global train_loss, reg_, forward_pass_time, backward_pass_time
        ...
        for step_num  in pbar:
            ...
            if step_num  == steps-1 and old_save_act:
                self.save_act = True
            
            train_id = np.random.choice(dataset['train_input'].shape[0], batch_size, replace=False)
            test_id = np.random.choice(dataset['test_input'].shape[0], batch_size_test, replace=False)

            if step_num % grid_update_freq == 0 and step_num < stop_grid_update_step and update_grid and step_num >= start_grid_update_step:
            ...
            if save_fig and step_num % save_fig_freq == 0:
                self.plot(folder=img_folder, in_vars=in_vars, out_vars=out_vars, title="Step {}".format(step_num), beta=beta)
                plt.savefig(img_folder + '/' + str(step_num) + '.jpg', bbox_inches='tight', dpi=200)
                plt.close()
