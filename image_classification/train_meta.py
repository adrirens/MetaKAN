...
def main():
    ...
    args.exp_id = args.exp_id + f"{'_'.join([str(w) for w in args.layers_width])}__{args.batch_norm}__{args.batch_size}__{args.epochs}__{args.seed}__{args.scheduler}"
    if args.optim_set == 'double':
        args.exp_id = args.exp_id + f"__{args.optim_set}__{args.lr_h}__{args.lr_e}"
    elif args.optim_set == 'single':
        args.exp_id = args.exp_id + f"__{args.optim_set}__{args.lr}__{args.optimizer}"
    os.makedirs(args.exp_id, exist_ok = True)

    ################# id for KAN #################
    if args.model in ["MetaKAN"]:
